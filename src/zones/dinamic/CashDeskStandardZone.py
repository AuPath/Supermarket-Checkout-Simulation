...
class CashDeskStandardZone(CashDeskZone):
    ...
    def build(self):
        if self.cash_desks_number == 0:
            ...
            x = ((self.model.cash_desk_self_scan_zone.cash_desks_number - math.ceil((self.model.grid.height - self.model.shopping_zone.dimension - 2) / 2))*2 if self.model.cash_desk_self_scan_zone is not None else 0)
            x = (x if x >= 0 else 0) + 4
            y = 0
            for cash_desk in range(self.cash_desks_number):
                cell = self.model.add_occupied_cell("", CashDeskType.STANDARD)
